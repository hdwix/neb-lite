  ...
  describe('getRide', () => {
    ...
    it('floors negative fareAfterDiscount and converts nullables to strings', async () => {
      const ride = {
        ...
        status: ERideStatus.CANDIDATES_COMPUTED,
        distanceActualKm: 1,
        discountAmount: '9999', // larger than base fare to trigger Math.max(0, ...)
        appFeeAmount: undefined, // ensures nullish coalescing branch for fee
      ...
      const candidates = [
        {
          driverId: 'DR-100',
          status: 'PENDING',
          distanceMeters: undefined, // coerced to null through ??
          respondedAt: '2025-01-04T00:10:00Z',
          createdAt: '2025-01-04T00:01:00Z',
        },
      ];
      ...
      const req = {
        [REQUEST_CLIENT_KEY]: { sub: 'r2', role: EClientType.RIDER },
      } as any;

      const res = await controller.getRide(req, 'ride-3');

      expect(res.data.baseFare).toBe('3000.00');
      expect(res.data.discountAmountByDriver).toBe('9999.00');
      expect(res.data.fareAfterDiscount).toBe('0.00'); // floored at zero via Math.max
      expect(res.data.finalFare).toBe('0.00');
    ...
    it('formats fareAfterDiscount and finalFare when base fare path is taken', () => {
      const ride = {
        id: 'ride-format',
        riderId: 'r3',
        driverId: 'd3',
        ...
        distanceActualKm: 1.1111, // baseFare -> 3333.3 -> 3333.30 after toFixed
        discountAmount: 33.33, // discountAmountByDriver -> 33.33 -> 3333.3 - 33.33 = 3299.97
        appFeeAmount: 10.1, // finalFare -> 3299.97 + 10.1 = 3310.07
        createdAt: '2025-01-07T00:00:00Z',
      } as any;

      const response = (controller as any).toRideResponse(ride);

      expect(response.baseFare).toBe('3333.30');
      expect(response.fareAfterDiscount).toBe('3299.97');
      expect(response.finalFare).toBe('3310.07');
    });

    it('falls back to null when final fare cannot be computed', () => {
      const ride = {
        id: 'ride-negative-app-fee',
        riderId: 'r4',
        driverId: 'd4',
        pickupLongitude: 1,
        pickupLatitude: 1,
        ...
        distanceActualKm: 1,
        discountAmount: 0,
        // appFeeAmount so negative that fareAfterDiscount + appFeeAmount < 0 -> parseCurrency(null)
        appFeeAmount: -100000,
      ...
      expect(response.baseFare).toBe('3000.00');
      expect(response.fareAfterDiscount).toBe('3000.00');
      expect(response.finalFare).toBe('3000.00');
    });
  });
