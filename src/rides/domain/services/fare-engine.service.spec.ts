...
describe('FareEngineService', () => {
  ...
  it('calculates fare with rounded distance, discount, and app fee', () => {
    const result = service.calculateFare({ distanceKm: 5.1234, discountAmount: 1000 });
  ...
  it('throws when discount amount is invalid or exceeds base fare', () => {
    expect(() => service.calculateFare({ distanceKm: 3, discountAmount: -10 })).toThrow(
      BadRequestException,
    );
    expect(() => service.calculateFare({ distanceKm: 1, discountAmount: 4000 })).toThrow(
      BadRequestException,
    );
  });
