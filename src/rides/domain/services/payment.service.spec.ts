...
describe('PaymentService', () => {
  ...
  it('queues payment initiation and waits for completion', async () => {
    const detail = { id: 'detail-1', status: ERidePaymentDetailStatus.PENDING } as any;
    const outbox = { id: 'outbox-1', paymentDetailId: 'detail-1', rideId: ride.id } as any;

    ridePaymentDetailRepository.findByRideId.mockResolvedValue(null);
    ridePaymentDetailRepository.create.mockReturnValue(detail);
    paymentOutboxRepository.saveDetailAndOutbox.mockResolvedValue({ detail, outbox });
    ...
    const refreshedDetail = { ...detail, redirectUrl: 'http://redirect', token: 'abc' } as any;
    ...
    expect(waitUntilFinished).toHaveBeenCalledWith(queueEvents, expect.any(Number));
  ...
  it('processes outbox successfully and returns formatted result', async () => {
    ...
    (lastValueFrom as jest.Mock).mockResolvedValue({ data: { token: 'abc', redirect_url: 'url' } });
  ...
  it('marks outbox as failed and propagates error when payment request fails', async () => {
    const axiosError: AxiosError = {
      ...
      config: {},
      toJSON: () => ({}),
      response: { status: 500, data: { message: 'oops' }, statusText: 'err', headers: {}, config: {} },
    } as AxiosError;
    ...
    await expect(service.processOutbox(outbox.id)).rejects.toThrow('Payment API request failed');

    expect(paymentOutboxRepository.save).toHaveBeenCalledWith(
      expect.objectContaining({ status: PaymentOutboxStatus.Failed, lastError: expect.any(String) }),
    );
  ...
  it('applies notification to payment detail and determines outbox updates', async () => {
    ...
    const result = await service.applyNotification(ride, payload, paymentDetail);
