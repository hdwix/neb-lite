...
const mockQueue = { name: 'ride-queue', opts: { connection: {} } } as unknown as Queue;
...
const buildService = (overrides: Partial<ConfigService> = {}) => {
  ...
  const fareEngine = { calculateEstimatedFare: jest.fn() } as unknown as FareEngineService;
...
describe('RidesManagementService', () => {
  ...
  it('throws descriptive errors for missing ORS configuration and parsing issues', async () => {
    ...
    const { service: misconfiguredService, configService: missingConfig } = buildService();
    ...
    await expect(
      misconfiguredService.fetchRouteEstimates({ longitude: 1, latitude: 2 }, { longitude: 3, latitude: 4 }),
    ...
    await expect(
      service.fetchRouteEstimates({ longitude: 0, latitude: 0 }, { longitude: 1, latitude: 1 }),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('wraps axios errors when requesting route summaries', async () => {
    const error = { isAxiosError: true, message: 'boom', response: { status: 500, statusText: 'err' } } as any;
    ...
    await expect(
      service.fetchRouteEstimates({ longitude: 1, latitude: 2 }, { longitude: 3, latitude: 4 }),
    ).rejects.toBeInstanceOf(BadRequestException);
