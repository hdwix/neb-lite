...
import { RidesManagementService, RequestingClient } from './rides-management.service';
...
describe('RidesTrackingService', () => {
  ...
  describe('startRide', () => {
    it('validates presence and ownership', async () => {
      ...
      rideRepository.findById.mockResolvedValue({ ...baseRide, driverId: 'other' });
      await expect(
        service.startRide('ride-1', 'driver-1', driverLocation),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('rejects invalid ride states', async () => {
      rideRepository.findById.mockResolvedValue({ ...baseRide, status: ERideStatus.CANCELED });
      await expect(
        service.startRide('ride-1', 'driver-1', driverLocation),
      ).rejects.toBeInstanceOf(BadRequestException);

      rideRepository.findById.mockResolvedValue({ ...baseRide, status: ERideStatus.COMPLETED });
      await expect(
        service.startRide('ride-1', 'driver-1', driverLocation),
      ).rejects.toBeInstanceOf(BadRequestException);

      rideRepository.findById.mockResolvedValue({ ...baseRide, status: ERideStatus.ASSIGNED });
      await expect(
        service.startRide('ride-1', 'driver-1', driverLocation),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('starts ride and notifies when transitioned', async () => {
      rideRepository.findById
        ...
        .mockResolvedValueOnce({ ...baseRide, status: ERideStatus.TRIP_STARTED });
      ...
      const ride = await service.startRide('ride-1', 'driver-1', driverLocation);
  ...
  describe('recordTripLocation', () => {
    const requester: RequestingClient = { id: 'client-1', role: EClientType.DRIVER };
  ...
  describe('completeRide', () => {
    const requester: RequestingClient = { id: 'driver-1', role: EClientType.DRIVER };
    ...
    it('completes ride, calculates fare, and notifies', async () => {
      ...
      const ride = await service.completeRide('ride-1', requester, { driverLocation });
