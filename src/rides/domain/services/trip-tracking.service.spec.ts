...
import { TRIP_TRACKING_QUEUE_NAME, TripTrackingQueueJob } from '../constants/trip-tracking.constants';
...
describe('TripTrackingService', () => {
  ...
  it('records driver location with distance calculation', async () => {
    ...
    expect(result).toEqual({ totalDistanceMeters: 15, distanceDeltaMeters: 10 });
  ...
  it('gets latest locations and totals from state', async () => {
    ...
    await expect(service.getLatestLocation('ride-1', EClientType.DRIVER)).resolves.toEqual(
      expect.objectContaining({ longitude: 1 }),
    );
    await expect(service.getLatestLocation('ride-1', EClientType.RIDER)).resolves.toEqual(
      expect.objectContaining({ longitude: 2 }),
    );
  ...
  it('marks ride completed and enqueues flush', async () => {
    jest
      .spyOn(service as any, 'getState')
      .mockResolvedValue({ rideId: 'ride-1', totalDistanceMeters: 1, completed: false });
