...
describe('RideStatusHistoryRepository', () => {
  ...
  it('creates entity from partial', () => {
    const history = repository.create({ rideId: '1', toStatus: 'completed' } as any);
  ...
  it('saves and maps row', async () => {
    ...
    const saved = await repository.save(repository.create({ rideId: '2', toStatus: 'accepted' } as any));
  ...
  it('throws when save returns no rows', async () => {
    ...
    await expect(
      repository.save(repository.create({ rideId: '2', toStatus: 'accepted' } as any)),
